...
class Task {
    ...
    public function __construct($id, $title, $description, $deadline, $completed, $images = [])
    {
        $this->setID($id);
        $this->setTitle($title);
    ...
    // This is a form of validation that checks if the parameters in the database are being met
    ...
    public function setCompleted($completed){
        if(strtoupper($completed) !== "Y" && strtoupper($completed) !== "N"){
            throw new TaskException("Completed status must be Y or N");
        }
    ...
    public function setImages($images){
        if(!is_array($images)){
            throw new ImageException("Images is not an array");
        }
        $this->_images = $images;
    }

    // returns task as an array so it can be formatted into json later.
